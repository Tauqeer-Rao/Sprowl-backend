const CustomerUser = require('../../models/CustomerModels/customerUserSchema');
...
exports.createUser = async (req, res) => {
    try {
        ...
        const newUser = new CustomerUser(req.body);
        await newUser.save();
...
exports.updateUserById = async (req, res) => {
    try {
        ...
        const user = await CustomerUser.findByIdAndUpdate(req.params.userId, req.body, { new: true });
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
        res.status(200).json(user);
...
exports.deleteUserById = async (req, res) => {
    try {
        ...
        const user = await CustomerUser.findByIdAndDelete(req.params.userId);
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
        res.status(204).send();
